...
import { getBridgeAPI } from './bridge-api.js'
import { getNetworkConfig, getDestinationAsset, toUnitsNumber } from './network-config.js'
...
export default class WalletAccountRgb extends WalletAccountReadOnlyRgb {
  ...
  //
  // These use the UTEXO Gateway bridge API (signature-based auth, no API keys).
  // Ported from @utexo/rgb-sdk UTEXOWallet — kept here so the worklet never
  // needs to import the full rgb-sdk (which crashes bare-module-resolve).
  // ============================================================================

  /** @private */
  _getBridge () {
    if (!this._bridge) {
      const network = this._config.network === 'mainnet' ? 'mainnet' : 'testnet'
      this._bridge = getBridgeAPI(network)
      this._networkIdMap = getNetworkConfig(network).networkIdMap
    }
    return this._bridge
  }

  /**
   * Creates a Lightning invoice for receiving RGB assets.
   *
   * @param {Object} options
   * @param {Object} options.asset - { assetId, amount }
   * @returns {Promise<Object>} Bridge transfer data with invoice
   */
  async createLightningInvoice (options) {
    const bridge = this._getBridge()
    const asset = options.asset
    if (!asset?.assetId || !asset?.amount) {
      throw new Error('asset.assetId and asset.amount are required')
    }

    const destinationAsset = getDestinationAsset(
      'mainnet', 'utexo', asset.assetId, this._networkIdMap
    )
    if (!destinationAsset) throw new Error('Destination asset not supported')

    const invoice = await this._wallet.witnessReceive({
      assetId: '',
      amount: asset.amount
    })

    const bridgeTransfer = await bridge.getBridgeInSignature({
      sender: {
        address: 'rgb-address',
        networkName: this._networkIdMap.mainnet.networkName,
        networkId: this._networkIdMap.mainnet.networkId
      },
      tokenId: destinationAsset.tokenId,
      amount: String(asset.amount),
      recipient: {
        address: invoice.invoice,
        networkName: this._networkIdMap.utexo.networkName,
        networkId: this._networkIdMap.utexo.networkId
      }
    })
  ...
  /**
   * Pays a Lightning invoice by sending RGB assets through the bridge.
   *
   * @param {Object} options
   * @param {string} options.lnInvoice - Lightning invoice to pay
   * @returns {Promise<Object>} Send result
   */
  async payLightningInvoice (options) {
    const bridge = this._getBridge()
    const bridgeTransfer = await bridge.getTransferByMainnetInvoice(
      options.lnInvoice,
      this._networkIdMap.mainnetLightning.networkId
    )
    if (!bridgeTransfer) {
      throw new Error('Lightning invoice not found on bridge')
    }

    const utexoInvoice = bridgeTransfer.recipient.address
    const destinationAsset = this._networkIdMap.utexo.getAssetById(
      bridgeTransfer.recipientToken.id
    )
    if (!destinationAsset) throw new Error('Destination asset not supported')

    const amount = toUnitsNumber(bridgeTransfer.recipientAmount, destinationAsset.precision)
  ...
  /**
   * Creates an onchain receive request via the bridge.
   *
   * @param {Object} options
   * @param {string} [options.assetId] - Asset to receive
   * @param {number} options.amount - Amount to receive
   * @returns {Promise<Object>} Bridge transfer data
   */
  async onchainReceive (options) {
    const bridge = this._getBridge()
    const destinationAsset = getDestinationAsset(
      'mainnet', 'utexo', options.assetId || null, this._networkIdMap
    )
    if (!destinationAsset) throw new Error('Destination asset not supported')
  ...
  /**
   * Sends RGB assets through the onchain bridge.
   *
   * @param {Object} options
   * @param {string} options.invoice - Mainnet invoice
   * @returns {Promise<Object>} Send result
   */
  async onchainSend (options) {
    const bridge = this._getBridge()
    const bridgeTransfer = await bridge.getTransferByMainnetInvoice(
      options.invoice,
      this._networkIdMap.mainnet.networkId
    )
    if (!bridgeTransfer) {
      throw new Error('Bridge transfer not found for invoice')
    }
  ...
  /**
   * Lists Lightning payment history via the bridge.
   *
   * @returns {Promise<Array>} Array of payment records
   */
  async listLightningPayments () {
    const bridge = this._getBridge()
    const { data } = await bridge.http.get('/v1/utexo/bridge/transfers/history', {
      params: {
        network_id: String(this._networkIdMap.mainnetLightning.networkId),
        offset: '0',
        limit: '50',
        address: 'rgb-address'
      }
